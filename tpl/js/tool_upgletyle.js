...
function _deleteTrackbackItem(params){
	...
	exec_xml('upgletyle', 'procUpgletyleTrackbackItemDelete', params, completeReload, response_tags);
}
...
function completeInsertComment(ret_obj) {
	...
	var url = current_url.setQuery('mid',mid).setQuery('act','dispUpgletyleToolCommunicationComment');
	location.href = url;
}
...
function _deleteCommentItem(params){
	var response_tags = new Array('error','message','page','mid');
	exec_xml('upgletyle', 'procUpgletyleCommentItemDelete', params, completeReload, response_tags);
}
...
function _deleteNotifyItem(params){
	var response_tags = new Array('error','message','page','mid');
	exec_xml('upgletyle', 'procUpgletyleNotifyItemDelete', params, completeReload, response_tags);
}
...
function updateCommentItemSetSecret(srl,is_secret,page,module_srl){
	...
	var response_tags = new Array('error','message','page','mid');
	exec_xml('upgletyle', 'procUpgletyleCommentItemSetSecret', params, completeReload, response_tags);
}
...
function _updateGuestbookItemChangeSecret(params){
	var response_tags = new Array('error','message','page','mid');
	exec_xml('upgletyle', 'procUpgletyleGuestbookItemsChangeSecret', params, completeGuestbookItemDelete, response_tags);
}
...
function completeInsertGuestbookReply(ret_obj){
	...
	var page = ret_obj['page'];
	var mid = ret_obj['mid'];
	location.href=current_url.setQuery('act','dispUpgletyleToolCommunicationGuestbook').setQuery('mid',current_mid);
}
...
function _deleteGuestbookItem(params){
	var response_tags = new Array('error','message','page','mid');
	exec_xml('upgletyle', 'procUpgletyleGuestbookItemsDelete', params, completeGuestbookItemDelete, response_tags);
...
function deleteDenyItem(mid,srl){
	...
	params['mid'] = mid;
	var response_tags = new Array('error','message','page','mid');
	exec_xml('upgletyle', 'procUpgletyleDenyDelete', params, completeInsertDeny, response_tags);
}
...
function completePostsave(ret_obj, response_tags, args, fo_obj) {
    ...
    var document_srl = ret_obj['document_srl'];
    var mid = ret_obj['mid'];
    var preview = ret_obj['preview'];
    var type = ret_obj['type'];

	fo_obj.document_srl.value = ret_obj.document_srl;
	alert(ret_obj.message);

	if(type == 'save') var url = current_url.setQuery('mid',mid).setQuery('act','dispUpgletyleToolPostManageWrite').setQuery('document_srl',document_srl).setQuery('preview',preview);
	else if(type =='publish') var url = current_url.setQuery('mid',mid).setQuery('act','dispUpgletyleToolPostManageList').setQuery('document_srl','');
...
function completePostwrite(ret_obj, response_tags, args, fo_obj) {
	...
	var url = current_url.setQuery('act','dispUpgletyleToolPostManageList');
	document.location.href = url;
}
...
function checkAlias(value,alias){
	...
	exec_xml('upgletyle','dispUpgletylePostCheckAlias',{'mid':current_mid,'alias':value},completeCheckAlias,response_tags);
}
...
function _restorePostItem(params){
	...
	exec_xml('upgletyle', 'procUpgletylePostTrashRestore', params, completeReload, response_tags);
...
function _trashPostItem(params){
	 var response_tags = new Array('error','message','page','mid');
	 exec_xml('upgletyle', 'procUpgletylePostTrash', params, completeReload, response_tags);
}
...
function updatePostItemsSecret(set_secret,page){
	...
	var response_tags = new Array('error','message','page','mid');
	exec_xml('upgletyle', 'procUpgletylePostItemsSetSecret', params, completeGuestbookItemDelete, response_tags);
}
...
function updatePostItemsCategory(vid,category_srl,page){
	...
	var response_tags = new Array('error','message','page','mid');
	exec_xml('upgletyle', 'procUpgletylePostItemsCategoryMove', params, completeGuestbookItemDelete, response_tags);
}
...
function deleteTag(obj){
	...
	var response_tags = new Array('error','message','page','mid');
	exec_xml('upgletyle', 'procUpgletyleTagDelete', params, completeDeleteTag, response_tags);
}
...
function updateTag(obj){
	...
	exec_xml('upgletyle', 'procUpgletyleTagUpdate', params, completeUpdateTag, response_tags);
}
...
function doSelectSkin(skin) {
	...
	params['skin'] = skin;
	params['mid'] = current_mid;
	exec_xml('upgletyle', 'procUpgletyleToolLayoutConfigSkin', params, completeReload, response_tags);
}
...
function doSelectMobileSkin(mskin) {
	...
	params['mskin'] = mskin;
	params['mid'] = current_mid;
	exec_xml('upgletyle', 'procUpgletyleToolLayoutConfigMobileSkin', params, completeReload, response_tags);
}
...
function doResetLayoutConfig() {
	...
	params['mid'] = current_mid;
	params['vid'] = xeVid;
	exec_xml('upgletyle', 'procUpgletyleToolLayoutResetConfigSkin', params, completeReload, response_tags);
}
...
function completeUpgletyleLogin(ret_obj, response_tags, params, fo_obj) {
    ...
    location.href = current_url.setQuery('act','dispUpgletyleToolDashboard');
}
...
function doPreProcessing(fo_obj) {
    ...
    exec_xml('upgletyle','procUpgletyleToolImportPrepare', params, completePreProcessing, response_tags);
...
function doImport() {
    ...
    exec_xml('upgletyle','procUpgletyleToolImport', params, completeImport, response_tags);
...
function doCheckTwitter() {
	...
	exec_xml('upgletyle', 'procUpgletyleCheckTwitter', params, function() {});
}
...
function doBlogApiTest() {
    ...
    exec_xml('upgletyle','getUpgletyleAPITest', params, completeBlogApiTest, new Array('error','message','site_url','title'));
}
...
function doCheckApiConnect() {
    ...
    exec_xml('upgletyle','getUpgletyleConnectInfo', params);
}
...
function doToggleAPI(obj, api_srl) {
    exec_xml('upgletyle','procUpgletyleToggleEnableAPI', {api_srl:api_srl}, function(ret_obj) { var o = jQuery(obj); if(ret_obj['enable']=='Y') o.attr('class','buttonSet buttonActive'); else o.attr('class','buttonSet buttonDisable'); }, new Array('error','message','enable'));
}

function doRemoveApi(api_srl) {
    exec_xml('upgletyle','procUpgletyleDeleteBlogApi', {api_srl:api_srl}, completeReload, new Array('error','message'));
}
...
function getBlogApiServiceInfo(srl) {
    ...
    exec_xml('upgletyle', 'getBlogApiService', params, function(data){
...
function initUpgletyle() {
	var params = new Array();
	params['mid'] = current_mid;
	params['vid'] = xeVid;

    exec_xml('upgletyle','procUpgletyleToolInit', params, 
		function(ret_obj){
			alert(ret_obj['message']);
			location.href = current_url.setQuery('act','dispUpgletyleToolDashboard');
...
function deleteUserImage(filename){
    ...
    jQuery.exec_json('upgletyle.procUpgletyleToolUserImageDelete', params, function(data){
        document.location.reload();
    });
}


function completeRequestExportUpgletyle(ret_obj){
	var error = ret_obj['error'];
	var message = ret_obj['message'];

	location.reload();
}
function completeExportUpgletyle(ret_obj){
	var error = ret_obj['error'];
	var message = ret_obj['message'];
...
validator.cast('ADD_CALLBACK', ['save_post', function callback(form) {
	...
	responses = ['error','message','mid','document_srl','category_srl', 'preview', 'type'];
	exec_xml('upgletyle','procUpgletylePostsave', params, completePostsave, responses, params, form);
...
function deleteExtraMenu(menu_mid,confirm_msg){
	...
	exec_xml('upgletyle', 'procUpgletyleToolExtraMenuDelete', params, completeReload, response_tags);
	}
}
...
function completeDeleteExtraMenu(ret_obj, response_tags, params, fo_obj) {
    ...
    location.href = current_url.setQuery('act','dispUpgletyleToolExtraMenuList');
}
...
function sortExtraMenu(menu_mids){
	...
	exec_xml('upgletyle', 'procUpgletyleToolExtraMenuSort', params, function(){}, response_tags);
}

function isLive(){
	exec_xml('upgletyle', 'procUpgletyleToolLive', []);
}
